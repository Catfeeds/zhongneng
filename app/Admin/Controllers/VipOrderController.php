...
use App\Models\Category,App\Models\VipOrder,App\Models\User;
...
class VipOrderController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return Admin::content(function (Content $content) use ($request){
            ...
            $content->header('vip购买记录');
            $content->description('vip购买记录');
    ...
    {
        return Admin::grid(VipOrder::class, function (Grid $grid) use ($request){
            ...
            //     $row = $actions->row;
            //     //课程按钮添加
            //     $actions->prepend('<a href="/admin/VipOrder-course?VipOrder_id='.$row['VipOrder_id'].'"><i class="fa fa-file-VipOrder-o"></i></a>');
            // });
            // if($request['cate_id']>0){
            //     $grid->urlCreateButton('/admin/VipOrder/create?cate_id='.$request['cate_id']);//修改添加按钮链接
            // }
            //筛选
    ...
    {
        return Admin::form(VipOrder::class, function (Form $form) use ($request){
            ...
            $form->saving(function (Form $form) {
                ...
                // if($width>0||$height>0){
                    $form->img = Image($form->img,537,289,"uploads/VipOrder/".date("Ymd")."/");
                // }
            });
