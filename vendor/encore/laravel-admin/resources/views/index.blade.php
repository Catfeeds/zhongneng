...
<head>
    ...
    <link href="{{asset('/resources/admin/css/zTreeStyle.css')}}" rel="stylesheet" />
    ...
    <script src="{{asset('/resources/admin/js/jquery.ztree.all.min.js')}}"></script>
    <!--[if lt IE 9]>
...
<body class="hold-transition {{config('admin.skin')}} {{join(' ', config('admin.layout'))}} @if(request()->collapse) sidebar-collapse @endif">
<div class="wrapper">
    @if(empty(request()->no_header))
    @include('admin::partials.header')
    @endif
    @if(empty(request()->no_sidebar))
